...
// Simulated video data for enrichment
const SIMULATED_VIDEOS: Record<string, { title: string; channelId: string; channelTitle: string; privacyStatus: string }> = {
...
// Simulated like counts for comments
const SIMULATED_LIKES: Record<string, number> = {
...
export function getSimulatedVideoInfo(videoId: string) {
	return SIMULATED_VIDEOS[videoId] || null;
}
...
export function getSimulatedLikeCount(commentId: string): number {
	return SIMULATED_LIKES[commentId] || Math.floor(Math.random() * 100);
}
